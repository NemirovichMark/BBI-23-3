...
namespace лаба_6_уровень_2_номер_3
{
    class Program
    {
        struct Participant
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < participants.Length - 1; i++)
            {
                ...
                {
                    if (Math.Max(Math.Max(participants[j].Res1, participants[j].Res2), participants[j].Res3) > Math.Max(Math.Max(participants[i].Res1, participants[i].Res2), participants[i].Res3))
                    {
